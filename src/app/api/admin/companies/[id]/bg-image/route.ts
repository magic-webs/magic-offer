...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data } = await adminDb.storage.uploadFile(`companies/${companyId}/bg-image`, buffer, {
    contentType: file.type || "image/png",
  });
  await adminDb.transact(adminDb.tx.companies[companyId].link({ bgImage: data.id }));
